process FASTQC {
    ...
    */
    ...
    """
    # Using integer arithmetic: 1 GiB = 1024 MiB
    memory_in_mb=\$(( \$FovusOptVcpuMem * 1024 / \$FovusOptVcpu ))

    # Clamp FastQC memory value to allowed range (100 - 10000 MB)
    if [ \$memory_in_mb -gt 10000 ]; then
        fastqc_memory=10000
    elif [ \$memory_in_mb -lt 100 ]; then
        fastqc_memory=100
    else
        fastqc_memory=\$memory_in_mb
    fi
